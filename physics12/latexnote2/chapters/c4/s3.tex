...
Visualizing Electric Fields - Field lines show how a small \underline{positive} charge would move.
...
\begin{equation}
    F_E = q \times \mathcal{E}
\end{equation}

\begin{center}
    $F_E$ is the magnitude of the electrical force exerted on $q$ (in $N$)\\
    $q$ is the that is \underline{in the electric field}(in $C$)\\
    $\mathcal{E}$ is the strength of the electrical field that the charge is in (in $\frac{N}{C}$)
\end{center}