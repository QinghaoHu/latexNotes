...
The kinetic energy an object has because it is \textit{moving from one location to another}. It can be described by this 
formula:
\begin{equation}
    E_k = \frac{1}{2}mv^2
\end{equation}
...
\end{equation}
\begin{center}
    $W_{A/B}$ is the work that $F_{A/B}$ does on object $B$. This is also the amount of $E_k$ that object A transfers to object B 
    when A exerts a force on B(in J)\\

    $F_{A/B}$ is the magnitude of force that A exerts on B (in N)\\
...
\begin{remark}
    ...
    You should always write $\cos \theta$ when using the formula
\end{remark}
