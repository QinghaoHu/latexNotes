\section{The Electric Field Between Two Charged Parallel Plates}
The electric field between two parallel plates is \underline{uniform}.\\
...
\subsubsection{Derivation of $\mathcal{E} = \frac{\Delta V}{d}$}
Consider a negative charge $q$ that starts at the positive plate and is pushed at a constant velocity toward the negative plate by an external force.
...
To start, let us solve for $F_{ap}$.
\begin{align}
    \sum \vec{F} &= 0 \nonumber \\
    F_{ap} - F_{E} &= 0 \nonumber \\
    F_{ap} &= F_{E} \nonumber \\
...
Next, let us solve for $W_{F_{ap}}$.
\begin{align}
...
When we deal with two charged parallel plates, we can say:
\begin{align*}
    \Delta E_E &= -\Delta E_k
\end{align*}
...
\begin{equation}
    \Delta E_E = q\Delta V \left(\frac{x}{d}\right)
\end{equation}
\begin{center}
    $d$ is the total separation between the two parallel plates\\
    $x$ is the distance the charged particle will move
\end{center}
