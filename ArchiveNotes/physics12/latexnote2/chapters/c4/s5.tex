...
\begin{definition}
    [Electrical Potential Difference] The change in electrical potential between two points, denoted by $\Delta V$.
\end{definition}

\subsubsection{Formula}
Assume a charge $q$ moves from a location where the electrical potential is $V_1$ to a location where the electrical potential is $V_2$.
\begin{figure}[!h]
    \centering
...
\end{align}
\begin{remark}
    When using this formula, substitute all signs.
\end{remark}

\begin{theorem}
    Assume we have two charges, $q_A$ (which is positive) and $q_B$ (which is negative). The total electrical potential at a point between $q_A$ and $q_B$ will be $V_A + V_B$.\\

    If $\left| q_A \right| = \left| q_B\right|$ and $q_B = -q_A$, the electrical potential at the midpoint between these two charges is 0.
...
If we place a new charge at the midpoint between $q_A$ and $q_B$, we find that $V = 0$ and $E_E = 0$. However, the charge would still move. To understand what happens to this charge, we need to examine the energy gradient.
\\

The charge will roll downward.
