...
The \textbf{formula works} best for two objects whose \red{separation distance} is \red{very large} relative to their sizes, 
or when both objects are perfect \red{spheres}.\\

The \textbf{formula works well} for a very large sphere whose mass is uniformly distributed throughout
...
A \red{gravitational field} is a region surrounding an object in which the object is capable of exerting a force of gravity 
on another object.
...
\begin{center}
    ...
    What does it imply?:  \blue{When an object is in free fall, it will accelerate at that rate}\\
    When is it true: \blue{Gravity exerts a force of $\mid \vec{g} \mid$ newtons on each kilogram of mass}
\end{center}
...
When you need to calculate the \blue{magnitude} of the gravitational field that object $M$ exerts on $m$:
...
\begin{center}
    ...
    $R$ is the distance from the location to $M$'s centre of mass (in m)\\
    $G$ is the universal gravitational constant ($G = 6.67\times 10^{-11}\frac{Nm^2}{Kg^2}$)
\end{center}
